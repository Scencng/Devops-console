...
func NewController() *Controller {
	return &Controller{service: serviceKafka.NewService(configs.NewKafkaClusterRepository()), auditRepo: configs.NewKafkaAuditLogRepository()}
}
...
func (c *Controller) ListClusterOptions(ctx *gin.Context) { helper := utils.NewResponseHelper(ctx); data, err := c.service.ListClusterOptions(); if err != nil { helper.InternalError(err.Error()); return }; helper.SuccessWithData("查询成功", "data", data) }

func (c *Controller) GetCluster(ctx *gin.Context) {
	helper := utils.NewResponseHelper(ctx)
	id, err := parseIDParam(ctx, "id")
	if err != nil { helper.BadRequest("无效的集群 ID"); return }
	data, err := c.service.GetCluster(id)
	if err != nil {
		if errors.Is(err, gorm.ErrRecordNotFound) { helper.NotFound("Kafka 集群不存在"); return }
		helper.InternalError(err.Error()); return
	}
	helper.SuccessWithData("查询成功", "data", data)
}
...
func (c *Controller) UpdateCluster(ctx *gin.Context) {
	...
	id, err := parseIDParam(ctx, "id")
	if err != nil { helper.BadRequest("无效的集群 ID"); return }
	before, _ := c.service.GetCluster(id)
	var req reqKafka.ClusterUpsertRequest
	if err = ctx.ShouldBindJSON(&req); err != nil { helper.BadRequest("请求参数错误: " + err.Error()); return }
	data, err := c.service.UpdateCluster(id, req)
	if err != nil {
		resourceName := req.Name
		if before != nil { resourceName = before.Name }
...
func (c *Controller) writeAuditLog(ctx *gin.Context, clusterID uint, action, resourceType, resourceName string, payload interface{}, result, errorMessage string) {
	...
	_ = c.auditRepo.Create(&dal.KafkaAuditLog{ClusterID: clusterID, Action: action, ResourceType: resourceType, ResourceName: resourceName, OperatorUserID: uint64(utils.GetUserIdFromContext(ctx)), OperatorUsername: utils.GetUserNameFromContext(ctx), RequestPayload: string(payloadBytes), Result: result, ErrorMessage: errorMessage, CreatedAt: time.Now()})
}
